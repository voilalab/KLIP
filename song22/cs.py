...
def get_projection_sampler(config, sde, model, shape, predictor, corrector,
  ...
  def get_inpaint_update_fn(update_fn):
    def inpaint_update_fn(rng, state, x, t, mask, known, coeff):
      ...
      _, g = sde.sde(known, t)
      return x, diff/g[:, None, None, None]
  ...
  def projection_sampler(rng, state, img, coeff, snr):
    ...
    def loop_body(i, carry):
      ...
      rng, step_rng = random.split(rng)
      x, diffs = cs_corrector_update_fn(step_rng, state, x, vec_t, mask, known, coeff)
      rng, step_rng = random.split(rng)
      x, _ = cs_predictor_update_fn(step_rng, state, x, vec_t, mask, known, coeff)

      buf0 = jax.lax.cond(
        i % diff_every == 0,
        lambda b: b.at[i // diff_every].set(diffs),
        lambda b: b,
        buf0,
      )
    ...
    buf0 = jnp.zeros((n_diffs,) + x.shape, dtype=x.dtype)
    rng, x, all_diffs = jax.lax.fori_loop(0, sde.N, loop_body, (rng, x, buf0))
    ...
    return inverse_scaler(output), all_diffs
