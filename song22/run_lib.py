...
def evaluate(config,
  ...
  raw = checkpoints.restore_checkpoint(checkpoint_dir, target=None)

  old_opt   = raw["optimizer"]
  params    = old_opt["target"]              # weights
  params = flax.serialization.from_state_dict(initial_params, params)
  step      = raw.get("step", 0)
  model_st  = raw.get("model_state", {})     # batch-norm / dropout stats
  params_ema= raw.get("params_ema", params)

  tx = optax.adamw(learning_rate=config.optim.lr,
    b1=config.optim.beta1,
    b2=getattr(config.optim, "beta2", 0.999),
  ...
  state = mutils.State(
    ...
    params      = params,
    tx          = tx,
    opt_state   = opt_state,     # field inherited from TrainState
    lr          = config.optim.lr,
    model_state = model_st,
    ema_rate    = config.model.ema_rate,
    params_ema  = params_ema,
    rng         = rng,   # or restore your saved rng
  )
